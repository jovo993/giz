...
class CustomAuditLogListener extends AuditLogListener {
  ...
  CustomAuditLogListener(Datastore datastore) {
    super(datastore)
  }
   ...
   *
   * TODO - Need a way to load the old value generically for a collection and embedded properties
   */
  ...
  protected Set<String> getDirtyPropertyNames(domain, GrailsDomainClass entity) {
    ...
    // For some reason, embedded properties are also not listed as being dirty by default.
    entity?.embedded.each { embeddedPropertyName ->
      def embeddedProperty = domain."$embeddedPropertyName"
      if (embeddedProperty?.respondsTo('isDirty') && embeddedProperty?.isDirty()) {
        dirtyProperties << embeddedPropertyName
      }
    }
  ...
  protected boolean significantChange(domain, Map oldMap, Map newMap) {
    ...
    oldMap.each { String k, Object v ->
      if (isPropertyAuditable(domain, k, auditableProperties, ignore) && v != newMap[k]) {
        changed = true
      }
    }
    return changed
  ...
  def logChanges(domain, Map newMap, Map oldMap, persistedObjectId, eventName, className) {
    ...
    if (newMap && oldMap) {
      ...
      newMap.each { String key, val ->
        if (!isPropertyAuditable(domain, key, auditableProperties, ignoreList)) {
          return
        }
        if (val != oldMap[key]) {
  ...
  */
   ...
   */
  boolean isPropertyAuditable(def domain, def fieldName, List auditableProperties, List ignoreList) {
    // TODO: Think of a better solution. This is a dirty hack to allow auditing of embedded fields only.
    if (getDomainClass(domain).embedded?.contains(fieldName)) {
